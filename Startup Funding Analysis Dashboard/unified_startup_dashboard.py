...
def render_performance(data):
    ...
    if (
        "Monthly Revenue" in data.columns
        and "Annual Burn Rate" in data.columns
    ):
        ...
        fig2 = go.Figure()
        fig2.update_layout(
            title_text="Revenue vs Burn Rate – data unavailable"
        )
    ...
    if cac_col in data.columns and clv_col in data.columns:
        ...
        fig3 = go.Figure()
        fig3.update_layout(
            title_text="CLV vs CAC – data unavailable"
        )

    # ---- fig4: Active Users vs Revenue Growth ----
    if (
        "Active Users" in data.columns
        and "Revenue Growth Rate (%)" in data.columns
    ):
        ...
        fig4 = go.Figure()
        fig4.update_layout(
            title_text="Active Users vs Revenue Growth – data unavailable"
        )

    # ---- fig5: Profit Margin by Industry ----
    if (
        "Profit Margin (%)" in data.columns
        and "Industry" in data.columns
    ):
        top_industries = (
        ...
        df_top = data[
            data["Industry"].isin(top_industries)
        ]
        ...
        fig5 = go.Figure()
        fig5.update_layout(
            title_text="Profit Margin by Industry – data unavailable"
        )
...
def render_insights(data):
    ...
    if (
        "Revenue Growth Rate (%)" in data.columns
        and "Profit Margin (%)" in data.columns
    ):
        ...
        fig1.update_layout(
            title_text="Growth vs Profitability – data unavailable"
        )
    ...
    if "Amount" in data.columns:
        ...
        fig2.update_layout(
            title_text="Funding Outliers – data unavailable"
        )


    # ---- fig3: Funding by Stage ----

    if (
        "Investment_Type" in data.columns
        ...
        fig3.update_layout(
            title_text="Funding by Stage – data unavailable"
        )
    ...
    if (
        ...
        fig4.update_layout(
            title_text="Top Investors – data unavailable"
        )
...
def update_all(
    ...
    if investment:
        data = data[
            data["Investment_Type"].isin(investment)
        ]
    ...
    if data.empty:
        ...
        empty_panel = html.Div(
            ...
            [
                ...
                html.P(
                    "The selected filters returned no records. "
                    "Please change the filters or click Reset Filters.",
    ...
    industry_chart["color"] = (
        industry_chart["Amount"]
        .apply(
            lambda x:
            "#FF9F1C"
            if x == max_ind
    ...
    others = pd.DataFrame({
        ...
        "Amount": [
            invest["Amount"].iloc[5:].sum()
        ]

    })

    invest_final = pd.concat(
        [top, others],
        ignore_index=True
    )
        ...
        [
            ...
            html.Ul(
                ...
                [
                    ...
                    html.Li(
                        f"Top City: {top_city_name} received the highest funding."
                    ),

                    html.Li(
                        f"Top Industry: {top_industry_name} attracted the most investment."
                    ),

                    html.Li(
                        f"Most common investment type: {top_type_name}."
                    ),

                    html.Li(
                        f"Total funding in selection: ${total:,.0f}."
                    ),
